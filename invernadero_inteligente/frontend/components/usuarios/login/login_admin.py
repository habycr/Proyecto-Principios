...
class Login:
    ...
    def iniciar_sesion(self):
        ...
        if respuesta.get("status") == "success":
            usuario = respuesta.get("user")
            # Verificar si el usuario es administrador (comparación case-insensitive)
            if usuario and usuario.get("rol", "").lower() == "administrador":
                self.usuario_autenticado = usuario
                self.mensaje_exito = "Inicio de sesión exitoso"
                self.mensaje_error = None
            else:
                self.mensaje_error = "La cuenta no tiene los permisos necesarios (se requiere rol Administrador)"
                self.usuario_autenticado = None
        else:
